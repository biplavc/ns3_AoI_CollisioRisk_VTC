...
Queue<Item>::DoEnqueue (ConstIterator pos, Ptr<Item> item)
{
  ...
  // std::cout<<"inside DoEnqueue in queue.h"<<std::endl; //Biplav

  if (GetCurrentSize () + item > GetMaxSize ())
    {
      NS_LOG_LOGIC ("Queue full -- dropping pkt");
      std::cout<<"inside DoEnqueue in queue.h where packets are dropped if no space"<<std::endl;
...
Queue<Item>::DoDequeue (ConstIterator pos)
{
  ...
  // std::cout<<"inside DoDequeue in queue.h"<<std::endl; //Biplav
  if (m_nPackets.Get () == 0)
    {
