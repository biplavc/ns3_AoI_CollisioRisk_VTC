...
WifiMacQueue::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiMacQueue")
    ...
    .AddAttribute ("MaxDelay", "If a packet stays longer than this delay in the queue, it is dropped.",
                   TimeValue (MilliSeconds (500)), // packets never expire if set to large value Biplav
...
WifiMacQueue::Enqueue (Ptr<WifiMacQueueItem> item)
{
  ...
  // std::cout<<"inside enqueue wifi-mac-queue.cc "<<std::endl; //Biplav
  ...
  if (QueueBase::GetNPackets () == GetMaxSize ().GetValue ())
    {
      auto it = Head ();
      while (it != Tail () && !TtlExceeded (it)) // it will point to the first, i.e. oldest expired message but what is it used to do ? //Biplav
        {
          it++;
        }
    }
  // std::cout<<"MaxSize = "<<GetMaxSize ().GetValue ()<<", m_dropPolicy = "<<m_dropPolicy <<std::endl; //Biplav
  if (QueueBase::GetNPackets () == GetMaxSize ().GetValue () && m_dropPolicy == DROP_OLDEST)
    {
      NS_LOG_DEBUG ("Remove the oldest item in the queue");
      DoRemove (Head ());
      // std::cout<<"Head dropped due to OldestDrop"<<std::endl; //Biplav
    }
...
{
  NS_LOG_FUNCTION (this << item);
  // std::cout<<"inside Pushfront wifi-mac-queue.cc"<<std::endl; //Biplav
  NS_ASSERT_MSG (GetMaxSize ().GetUnit () == QueueSizeUnit::PACKETS,
                 "WifiMacQueues must be in packet mode");
  ...
  if (QueueBase::GetNPackets () == GetMaxSize ().GetValue ())
    {
      auto it = Head ();
      while (it != Tail () && !TtlExceeded (it)) // it will point to the first, i.e. oldest expired message //Biplav
        {
          it++;
        }
    }
  ...
  if (QueueBase::GetNPackets () == GetMaxSize ().GetValue () && m_dropPolicy == DROP_OLDEST)
    {
      NS_LOG_DEBUG ("Remove the oldest item in the queue");
      DoRemove (Head ()); // should be tail Biplav
    }
...
{
  NS_LOG_FUNCTION (this);
  // std::cout<<"inside Dequeue in wifi-mac-queue.cc"<<std::endl; //Biplav
