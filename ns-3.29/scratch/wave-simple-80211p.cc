...
static Vector NeighborsLoc [50][50];
...
int main (int argc, char *argv[])
{
  ...
  for (int i=0; i<3; i++)
  {
    for (int j=0; j<2; j++)
    {
      std::cout<<NeighborsLoc[i][j]<<std::endl;
    }
  }
  NeighborsLoc[0][1] = Vector(3.0,2.0,0.0);
  ...
  // return 0;
  // static Time intervals[5] = {MilliSeconds(100)};
  ...
  // Time bb = MilliSeconds(100);
  // std::cout<<bb<<" "<<static_cast<uint32_t>(bb.GetInteger())<<std::endl;
  ...
  //   for (int j=0; j<2; j++)
  //   {
  //     out2<<NeighborsLoc[i][j]<<std::endl;
  //   }
  // }
  // out2.close ();
