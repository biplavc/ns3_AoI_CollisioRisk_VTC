...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  Time sqhd;
  Time total_delay;
  while ((packet = socket->Recv ()) && (Simulator::Now() > Seconds(203)))// start logging after 1865 seconds as in SUMO, 200 vehicles enter simulation after 173 seconds. For 50 vehicles, it is 173.
  {
...
//   ns3::Time::SetResolution(Time::NS);
  ...
  double intervals[] = {0.06666666666666667,0.04, 0.03333333333, 0.02857142857142857,0.022222222222222223,0.01818181818181818, 0.016666666666666666};
    ...
    for (int m = 0; m < mm; m++) // loop for number of vehicles
    {
        ...
        //   m_streamIndex += mobility.AssignStreams (c, m_streamIndex);
          ...
          for (int i = 0; i < nSinks; i++)
          {
              ...
              Simulator::ScheduleWithContext (source->GetNode ()->GetId (), Seconds (196+rv->GetValue()), &GenerateTraffic, source, packetSize, numPackets, interPacketInterval); //without rv, no packets delivered
          }
