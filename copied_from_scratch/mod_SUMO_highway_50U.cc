...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  Time sqhd;
  Time total_delay;
  while ((packet = socket->Recv ()) && (Simulator::Now() > Seconds(15)))// start logging after 1865 seconds as in SUMO, 200 vehicles enter simulation after 173 seconds. For 50 vehicles, it is 173.
  {
    ...
    for (int m = 0; m < mm; m++) // loop for number of vehicles
    {
        ...
        //   m_streamIndex += mobility.AssignStreams (c, m_streamIndex);
          ...
          for (int i = 0; i < nSinks; i++)
          {
              ...
              Simulator::ScheduleWithContext (source->GetNode ()->GetId (), Seconds (12+rv->GetValue()), &GenerateTraffic, source, packetSize, numPackets, interPacketInterval); //without rv, no packets delivered
          }
