...
// LogComponentEnable("Header Added", LOG_LEVEL_INFO);
...
int main (int argc, char *argv[])
{
  ...
  // uint32_t numPackets = 400; //packets to be sent per node
  ...
  // double intervals[] = {1.0, 0.25, 0.125, 0.084, 0.067, 0.05}; //intervals corresponding to 1Hz, 4Hz, 8Hz, 12Hz, 16Hz, 20Hz
  // double intervals[] = {1.0, 0.5, 0.333, 0.25, 0.2, 0.1667, 0.143, 0.125, 0.1113, 0.1, 0.091, 0.083, 0.077, 0.072, 0.0677, 0.0625, 0.059, 0.055, 0.053, 0.05}; // interval corresponding to 1 - 20Hz
...
//   double intervals[] = {1.0, 0.2, 0.1, 0.06666666666666667, 0.05, 0.04, 0.03333333333, 0.02857142857142857, 0.025, 0.022222222222222223, 0.02, 0.01818181818181818, 0.016666666666666666, 0.015384615384615385, 0.014285714285714285, 0.013333333333333334, 0.0125};
 //intervals as per      1,   5,   10,            15,        20,   25,        30            35,          40,             45,         50,            55,                   60,                  65,                  70,                    75,            80 Hz
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop over intervals
  {
    ...
    // std::cout<<(interPacketInterval.GetInteger())<<"\n"<<std::endl;
    // int arr[] = {20, 40, 60, 80, 100, 120, 140, 160, 180, 200}; //number of nodes in each scenario
    int arr[] = {250}; // denotes the number of vehicles
    // int arr[] = {100}; 
    ...
    for (int m = 0; m < mm; m++)
    {
      ...
      pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1000.0]")); //max value as per ROAD_LENGTH_NUM
      ...
      double veh_spacing = c.GetN()*num_lanes/ROAD_LENGTH_NUM;  
      ...
      for(unsigned int i=0;i<c.GetN(); i++)
        {
          ...
          curr_lane++;
          std::cout<<"vehicle "<<i<<"'s position is ("<<posm.x<<", "<< posm.y <<", " <<posm.z<<"), speed = ("<<vel.x<<", "<< vel.y <<", " <<vel.z<<")"<<std::endl;
      ...
      for (int i = 0; i < nSinks; i++)
        {
          ...
          Simulator::ScheduleWithContext (source->GetNode ()->GetId (),
                                          ...
                                          source, packetSize, numPackets, interPacketInterval); //without rv, no packets delivered as all packets will attempt at the same time
        }
      ...
      std::string file_name ("Test" + str_2 + "Hz_" + str + ".txt"); 
      ...
      std::string file_name_1 ("Error_Test" + str_2 + "Hz_" + str + ".txt"); 
      freopen(file_name_1.c_str(),"a",stderr);
