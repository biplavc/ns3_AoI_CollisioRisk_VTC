...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  while (packet = socket->Recv ())
  {
    ...
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<total_error<<" "<<sqhd.GetInteger()<<" "<<rcv.GetInteger()<<" "<<total_delay.GetInteger()<<xx<<" "<<yy<<" "<<zz<<"\n"<<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 3; //packets to be sent per node
...
//   int arr[] = {20, 40, 60, 80, 100, 120, 140, 160, 180, 200}; //number of nodes in each scenario
  ...
  for (int m = 0; m < mm; m++)
  {
    ...
    // ss << filename_append;
    // std::string str = ss.str();
    // std::string file_name ("GaussianXSpeed_X1K" + str + ".txt"); 
