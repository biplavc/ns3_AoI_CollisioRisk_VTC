...
WaveNetDeviceExample::CreateWaveNodes (void)
{
  ...
  YansWavePhyHelper wavePhy =  YansWavePhyHelper::Default (); //how to set rate?//
  ...
  for (uint32_t i = 0; i != devices.GetN (); ++i)
    {
      ...
      device->SetReceiveCallback (MakeCallback (&WaveNetDeviceExample::Receive, this)); //Receive needs some parameters but 
                                                                                        //not provided here so how will the call succeed?
      device->SetWaveVsaCallback (MakeCallback  (&WaveNetDeviceExample::ReceiveVsa, this)); //same as above//
    }
