...
MacTxDrop(Ptr<const Packet> p)
{
  ...
  MacTxDropCount++;
  std::cout<<"MAC drop at Tx"<<std::endl;
...
MacRxDrop(Ptr<const Packet> p)
{
  // NS_LOG_INFO("Packet Drop");
  MacRxDropCount++;
  std::cout<<"MAC drop at Rx"<<std::endl;
...
RoutingHelper::SetupRoutingProtocol (NodeContainer & c)
{
  ...
  switch (m_protocol)
    ...
    case 0:
      m_protocolName = "NONE";
      std::cout<<"NONE selected as m_protocolName"<<std::endl;
      break;
    case 1:
    ...
    case 2:
      ...
      list.Add (aodv, 100);
      m_protocolName = "AODV";
      std::cout<<"AODV selected as m_protocolName"<<std::endl;
      break;
    case 3:
      if (m_routingTables != 0)
        {
                                     ...
                                     Ipv4InterfaceContainer & adhocTxInterfaces)
{
  ...
  std::cout<<"SetupRoutingMessages called, doesn't execute for BSM only config "<<std::endl;
  ...
  for (uint32_t i = 0; i < m_nSinks; i++)
    {
      ...
      if (m_protocol != 0)
        {
          Ptr<Socket> sink = SetupRoutingPacketReceive (adhocTxInterfaces.GetAddress (i), c.Get (i)); //Biplav, for BSM the only receive packet callback is disabled. need to have a custom callback for reception of BSM ?
        }

      AddressValue remoteAddress (InetSocketAddress (adhocTxInterfaces.GetAddress (i), m_port));
      onoff1.SetAttribute ("Remote", remoteAddress);
      std::cout<<"This part doesn't get executed for BSM-only config"<<std::endl; //Biplav
...
PrintReceivedRoutingPacket (Ptr<Socket> socket, Ptr<Packet> packet, Address srcAddress)
{
  ...
  //called from ReceiveRoutingPacket, so doesn't execute for BSM only config
  oss << Simulator::Now ().GetSeconds () << " " << socket->GetNode ()->GetId ();
  // add the receive results section here
...
{
  // std::cout<<"Entered ReceiveRoutingPacket"<<std::endl; //Biplav
  Ptr<Packet> packet;
  Address srcAddress;
  while ((packet = socket->RecvFrom (srcAddress)))
    {
      // application data, for goodput
      std::cout<<"Inside loop of ReceiveRoutingPacket, doesn't get executed for BSM"<<std::endl;  //Biplav
      uint32_t RxRoutingBytes = packet->GetSize ();
      GetRoutingStats ().IncRxBytes (RxRoutingBytes);
      GetRoutingStats ().IncRxPkts ();
      if (m_log != 0)
        {
          NS_LOG_UNCOND (m_protocolName + " " + PrintReceivedRoutingPacket (socket, packet, srcAddress) + " in ReceiveRoutingPacket");
        }
    }
}
...
RoutingHelper::OnOffTrace (std::string context, Ptr<const Packet> packet)
{
  ...
  //std::cout<<"Packet generated by OnOff app" <<std::endl;
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    // m_nSinks (10), //Biplav, no sinks for broadcast
    ...
    m_protocol (0), //Biplav
    // Two-Ray ground
    m_lossModel (3), //TwoRayGroundPropagationLossModel
    m_fading (1), // Nakagami //Biplav
    m_lossModelName (""),
    m_phyMode ("OfdmRate6MbpsBW10MHz"),
    // 1=802.11p
    m_80211mode (3), //WAVE // Biplav 
    m_traceFile (""),
    m_logFile ("low99-ct-unterstrass-1day.filt.7.adj.log"),
    m_mobility (2), //further config needed Biplav
    m_nNodes (156),
    m_TotalSimTime (300.01),
    m_rate ("2048bps"),
    m_phyModeB ("DsssRate11Mbps"), // relates to sth in 802.11b
    ...
    m_waveInterval (0.1),
    m_verbose (0),
    m_scenario (1), //synthetic, further config needed
    m_gpsAccuracyNs (40),
    m_txMaxDelayMs (10),
    m_routingTables (0),
    m_asciiTrace (0),
    ...
    m_txSafetyRanges (),
    m_exp (""),
    m_cumulativeBsmCaptureStart (0) // after the simulation starts, how long to wait until BSM stats are captured
{
...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  double averageBSMGoodputKbps = 0.0; //Biplav
  ...
  averageBSMGoodputKbps = (((double) cumulativeWaveBsmBytes * 8.0) / m_TotalSimTime) / 1000.0;
  ...
  if (m_log != 0)
    {
      std::cout<<"BSM_PDR1=" << bsm_pdr1 << ", BSM_PDR2=" << bsm_pdr2 << ", BSM_PDR3=" << bsm_pdr3 << ", BSM_PDR4=" << bsm_pdr4 << ", BSM_PDR5=" << bsm_pdr5 << ", BSM_PDR6=" << bsm_pdr6 << ", BSM_PDR7=" << bsm_pdr7 << ", BSM_PDR8=" << bsm_pdr8 << ", BSM_PDR9=" << bsm_pdr9 << ", BSM_PDR10=" << bsm_pdr10 << ", BSM Goodput = " << averageBSMGoodputKbps << " Kbps"<<std::endl; // MAC/PHY-oh=" << mac_phy_oh);
...
VanetRoutingExperiment::CheckThroughput ()
{
  ...
  if (m_log != 0 )
    {
      NS_LOG_UNCOND ("At t=" << (Simulator::Now ()).GetSeconds () << "s BSM_PDR1=" << wavePDR1_2 << " BSM_PDR2=" << wavePDR2_2 << " BSM_PDR3=" << wavePDR3_2 << " BSM_PDR4=" << wavePDR4_2 << " BSM_PDR5=" << wavePDR5_2 << " BSM_PDR6=" << wavePDR6_2 << " BSM_PDR7=" << wavePDR7_2 << " BSM_PDR8=" << wavePDR8_2 << " BSM_PDR9=" << wavePDR9_2 << " BSM_PDR10=" << wavePDR10_2 << " Goodput=" << kbps << "Kbps  " /*<< " MAC/PHY-OH=" << mac_phy_oh*/);
    }
...
VanetRoutingExperiment::SetupAdhocMobilityNodes ()
{
  if (m_mobility == 1)
  ...
  else if (m_mobility == 2)
    {
      ...
      pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=1000]")); //Distance to 1 km replaced Biplav
      ...
      double veh_spacing=(1000.000)*num_lanes/m_adhocTxNodes.GetN(); //replaced Biplav
      for(unsigned int i=0;i<m_adhocTxNodes.GetN(); i++)
        {
          ...
          mob->SetPosition(Vector(x, lane_y[curr_lane], 0));
          ...
          std::cout << Simulator::Now () << " Setup Node 1st time: " << i << " POS: x=" << posm.x << ", y=" << posm.y << ", z=" << posm.z << "; VEL:" << vel.x << ", y=" << vel.y  << ", z=" << vel.z << std::endl;
          lane_pos[curr_lane] += veh_spacing;
          if (lane_pos[curr_lane] > 1000) //replaced Biplav
