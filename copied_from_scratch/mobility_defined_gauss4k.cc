...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  while (packet = socket->Recv ())
  {
    ...
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<total_error<<" "<<sqhd.GetInteger()<<" "<<rcv.GetInteger()<<"\n"<<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 10; //packets to be sent per node
  ...
  for (int m = 0; m < mm; m++)
  {
    ...
    pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=4000.0]"));
