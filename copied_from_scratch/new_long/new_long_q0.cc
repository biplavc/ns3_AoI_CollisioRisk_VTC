...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  while (packet = socket->Recv ())
  {
    ...
    Ptr<ConstantVelocityMobilityModel> mob_rec = socket->GetNode()->GetObject<ConstantVelocityMobilityModel>();
    double rec_x = mob_rec->GetPosition().x;
    double rec_vel = mob_rec->GetVelocity().x; 
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<total_error<<" "<<sqhd.GetInteger()<<" "<<rcv.GetInteger()<<" "<<total_delay.GetInteger()<<" "<<xx<<" "<<old_x<<" "<<xx1<<" "<<rec_vel<<" "<<rec_x<<"\n"<<std::endl;
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 8000; //packets to be sent per node
...
//   double intervals[] = {0.1};
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop over intervals
  {
    ...
    int arr[] = {50, 100, 150, 200}; //number of nodes in each scenario
    // int arr[] = {3}; // denotes the number of vehicles
    ...
    for (int m = 0; m < mm; m++)
    {
        ...
        std::string file_name ("New_long_" + str_2 + "Hz_" + str_1 + "U.txt"); 
        ...
        std::cout<<"PId TxId RxId Error(m) GenTime RecTime Delay TruePos(X) HeadPos(X) SenderVelocity ReceiverVelocity Receiver(X) \n"<<std::endl;
