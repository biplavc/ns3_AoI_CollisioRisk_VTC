...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  Time sqhd;
  Time total_delay;
  while ((packet = socket->Recv ()) && (Simulator::Now() > Seconds(178)))// start logging after 1865 seconds as in SUMO, 200 vehicles enter simulation after 173 seconds. For 50 vehicles, it is 173.
  {
    ...
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<(sqhd).GetMilliSeconds()<<" "<<(rcv).GetMilliSeconds()<<" "<<(total_delay).GetMilliSeconds()<<" "<<xx<<" "<<old_x<<" "<<yy<<" "<<old_y<<" "<<total_error<<" "<<xx1<<" "<<yy1<<" "<<xx2<<" "<<yy2<<"\n"<<std::endl;
  }
}
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop for beacon rates
  {
    ...
    for (int m = 0; m < mm; m++) // loop for number of vehicles
    {
        ...
        //   double veh_spacing = ROAD_LENGTH_NUM*num_lanes/c.GetN();
          ...
          std::string file_name ("Prog_" + queue_here + "_" + str_2 + "Hz_" + str_1 + "U.txt"); 
          ...
          std::cout<<"PId TxId RxId GenTime RecTime Delay TxTruePos(X) TxHeadPos(X) TxTruePos(Y) TxHeadPos(Y) Error(m) Tx_HeadVelX Tx_HeadVelY RecvPos(X) RecvPos(Y)\n"<<std::endl;
        ...
        //   {
          ...
          Ns2MobilityHelper ns2 = Ns2MobilityHelper ( "/home/biplav/circle_50U.tcl");
        ...
        //   m_streamIndex += mobility.AssignStreams (c, m_streamIndex);
          ...
          for (int i = 0; i < nSinks; i++)
          {
              ...
              Simulator::ScheduleWithContext (source->GetNode ()->GetId (), Seconds (176+rv->GetValue()), &GenerateTraffic, source, packetSize, numPackets, interPacketInterval); //without rv, no packets delivered
          }
