 ...
 */
class RoutingStats //why is this class not derived from object class?? Shouldn't all classes derive from Object??
{
public:
  /**
   * \brief Constructor
   ...
   */
  uint32_t GetRxBytes (); //difference between this and GetCumulativeRxBytes()
   ...
   */
  uint32_t GetRxPkts (); //difference with GetCumulativeRxPkts ()
...
//the next lines are the initalization table that we use in constructors, iitialize all packets and bytes to 0
...
//any significance of the difference in the ways byte is incremented vs the packet is incremented
void
...
{
  m_RxPkts++;
  m_cumulativeRxPkts++; // will always be incremented by 1 packet but the packet size will vary 
}
...
public:
   ...
   * \return none
   */
  virtual ~RoutingHelper (); /*usually in virtual destructors, we make the destructor of the base class virtual but here
                              the RoutingHelper is a derived class but has virtual destructor?? */
  /**
   ...
   * \param nSinks the number of nodes which will act as data sinks \\this method is to install, why do we need to specify no of data sinks here??
   ...
   * \return none \\checks whether the object has received the packet??
   */
  void OnOffTrace (std::string context, Ptr<const Packet> packet); //meaning of adding const here to packet??
  //what does this function do exactly?
  /**
   * \brief Returns the RoutingStats instance
   * \return the RoutingStats instance
   */
  //will return a reference of a RoutingStats object https://stackoverflow.com/questions/1943276/what-does-do-in-a-c-declaration
  RoutingStats & GetRoutingStats (); //what does the declaration line mean?? reference operator??
...
private:
  /**
   * \brief Sets up the protocol on the nodes
   * \param c node container
   * \return none
   */
  void SetupRoutingProtocol (NodeContainer & c); //we are just passing nodes, so which protocol to set?
  ...
  /**
   * \brief Sets up routing messages on the nodes and their interfaces //what does setting up routing msgs at node/interface mean??
   * \param c node container
   * \param adhocTxInterfaces IPv4 interface container
   * \return none
   */
...
NS_OBJECT_ENSURE_REGISTERED (RoutingHelper); //this has to be in the beginning or end??

//what exactly will happen when TypeId is called? SetParent and AddConstructor called automatically??
TypeId
RoutingHelper::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RoutingHelper")
    .SetParent<Object> () //whose parent we are setting?? What is Object?? 
                        ...
                        int routingTables)
{
  ...
  SetupRoutingProtocol (c); //Sets up the protocol on the nodes MEANS??
  AssignIpAddresses (d, i);
  SetupRoutingMessages (c, i);  //Sets up routing messages on the nodes and their interfaces MEANS??
}
...
RoutingHelper::SetupRoutingPacketReceive (Ipv4Address addr, Ptr<Node> node)
{
  ...
  Ptr<Socket> sink = Socket::CreateSocket (node, tid); //tid socket will be created at the node
  InetSocketAddress local = InetSocketAddress (addr, m_port); //local is just an address that has an IP address and a port
  sink->Bind (local);
  sink->SetRecvCallback (MakeCallback (&RoutingHelper::ReceiveRoutingPacket, this)); //why the reference operator here??
                                     ...
                                     Ipv4InterfaceContainer & adhocTxInterfaces)
{
  ...
  OnOffHelper onoff1 ("ns3::UdpSocketFactory",Address ()); //Generate traffic to a single destination according to an OnOff pattern.
  onoff1.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"));
  onoff1.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.0]")); //no off time what does that mean??

  Ptr<UniformRandomVariable> var = CreateObject<UniformRandomVariable> (); //The uniform distribution Random Number Generator (RNG)
  int64_t stream = 2;
  var->SetStream (stream); //??
  for (uint32_t i = 0; i < m_nSinks; i++)
    {
      // protocol == 0 means no routing data, WAVE BSM only IMPTT // BIPLAV
...
PrintReceivedRoutingPacket (Ptr<Socket> socket, Ptr<Packet> packet, Address srcAddress)
{
  ...
  if (InetSocketAddress::IsMatchingType (srcAddress)) //we are not passing the socketaddress so how does it get access to that??
    {
      InetSocketAddress addr = InetSocketAddress::ConvertFrom (srcAddress);
      oss << " received one packet from " << addr.GetIpv4 (); //non-broadcast scenario??
    }
  else
    {
      oss << " received one packet!"; //broadcast scenario??
    }
  return oss.str ();
}
...
RoutingHelper::ReceiveRoutingPacket (Ptr<Socket> socket)
{
  Ptr<Packet> packet;
  Address srcAddress;
  while ((packet = socket->RecvFrom (srcAddress))) //will go on running until all data received in 'packet' ??
    {
      ...
      if (m_log != 0) //automatically !0 for correct routing??
        {
          std::cout<<"printing from ReceiveRoutingPacket"<<std::endl;
...
public:
  ...
  /**
   * \brief Callback signiture for Phy/Tx trace MEANS??
   * \param context this object //it is a string so how come it is an object??
  ...
  /**
   * \brief Callback signiture for Phy/TxDrop MEANS??
   * \param context this object
  ...
  /**
   * \brief Callback signiture for Phy/RxDrop MEANS??
   * \param context this object
...
//WifiMode means?? doc mentions lookup table so which table? Means MCS??
//context is the string identifier or what does it mean??
void
WifiPhyStats::PhyTxTrace (std::string context, Ptr<const Packet> packet, WifiMode mode, WifiPreamble preamble, uint8_t txPower)
{
  std::cout<<"next lines from WifiPhyStats::PhyTxTrace"<<std::endl;
...
{
  std::<<"next lines from WifiPhyStats::PhyTxDrop"<<std::endl;
  NS_LOG_UNCOND ("PHY Tx Drop"); //means??
}

void
WifiPhyStats::PhyRxDrop (std::string context, Ptr<const Packet> packet)
{
  std::cout<<"next lines from WifiPhyStats::PhyRxDrop"<<std::endl;
...
public:
  ...
  /**
   * \brief Enacts simulation of an ns-3 wifi applicationWifiApp
...
protected:
   ...
   * \return none
   */
  virtual void SetDefaultAttributeValues (); //class WifiApp
...
WifiApp::Simulate (int argc, char **argv)
{
  ...
  SetDefaultAttributeValues (); //class WifiApp
 ...
 */
class VanetRoutingExperiment : public WifiApp //why is WiFiApp not declared as virtual, 
// when almost all its methods will be overwritten by these applications?? Instead 
// VanetRoutingExperiment methods are declared as virtual??
{
public:
  /**
   * \brief Constructor
...
protected:
   ...
   * \return none
   */
  virtual void SetDefaultAttributeValues (); //class VanetRoutingExperiment
...
private:
  ...
  // if returns a void, why static needed??
  static void
  ...
  std::string m_phyModeB; ///< phy mode //means ??
  ...
  double m_gpsAccuracyNs; ///< GPS accuracy //where set??
  ...
  int64_t m_streamIndex; //use of streamindex??
  NodeContainer m_adhocTxNodes; ///< adhoc transmit nodes //means??
  double m_txSafetyRange1; ///< range 1 //whAt are these??
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    m_nSinks (10),
    m_protocolName ("protocol"), // BIPLAV: set to NONE for BSM ?
    m_txp (20),
    m_traceMobility (false),
    // AODV
    m_protocol (0), // 0 means only BSM BIPLAV
    // Two-Ray ground
    m_lossModel (3), //??
    m_fading (0), //??
    m_lossModelName (""), //??
    ...
    m_waveInterval (0.1), // transmission rate BIPLAV
    m_verbose (0),
    m_scenario (1),
    m_gpsAccuracyNs (40),
    m_txMaxDelayMs (10),
    ...
    m_cumulativeBsmCaptureStart (0)
{
  ...
  // set to non-zero value to enable simply uncond logging during simulation run
  m_log = 1;
}
...
//for changing them, should I change from here?? #1
static ns3::GlobalValue g_port ("VRCport",
                                "Port",
                                ns3::UintegerValue (9),
                                ns3::MakeUintegerChecker<uint32_t> ());
static ns3::GlobalValue g_nSinks ("VRCnSinks",
                                  "Number of sink nodes for routing non-BSM traffic", //IMPTT: Sinks only for non-BSM
...
VanetRoutingExperiment::ParseCommandLineArguments (int argc, char **argv)
{
  ...
  m_txSafetyRanges.resize (10, 0); //all these are declared in the constructor of VanetRoutingExperiment
...
VanetRoutingExperiment::ConfigureDevices ()
{
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/Phy/State/Tx", MakeCallback (&WifiPhyStats::PhyTxTrace, m_wifiPhyStats)); //* means all //
  //Config::Connect(path, callback) - This function will attempt to find all trace sources which match the input path and will then 
  //connect the input callback to them in such a way that the callback will receive an extra context string upon trace event notification//
  //what does the extra string mean??
...
VanetRoutingExperiment::ConfigureApplications ()
{
  ...
  SetupRoutingMessages (); //cancel this initially ?? if this is ON with routing protocol 0 (broadcast), will the total data now include BSM and 
  // the other data that was meant to be routed??
  SetupWaveMessages ();
  ...
  std::ostringstream oss; //??
  oss.str (""); //??
  oss << "/NodeList/*/ApplicationList/*/$ns3::OnOffApplication/Tx"; //what does the Tx at the end mean?
...
{
  // calculate and output final results //#2 //Also see #3
  double bsm_pdr1 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (1); //PDR just for BSM??
  ...
  double bsm_pdr10 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPd#3r (10);
  //#3
  double averageRoutingGoodputKbps = 0.0;#3
  uint32_t totalBytesTotal = m_routingHelper->GetRoutingStats ().GetCumulat#3iveRxBytes ();
  averageRoutingGoodputKbps = (((double) totalBytesTotal * 8.0) / m_TotalSimTime) / 1000.0; //kilobits
  ...
  std::ofstream out (m_CSVfileName2.c_str (), std::ios::app); //writing out to the file??
  ...
  out << bsm_pdr1 << ","
      ...
      << mac_phy_oh << ""
      << std::endl;GetCumulativeBsmPdr
GetCumulativeBsmPdr
  out.close ();GetCumulativeBsmPdr
GetCumulativeBsmPdr
...
{VanetRoutingExperiment::ProcessOutputs ()
  std::cout<<"next line from VanetRoutingExperiment::RuVanetRoutingExperiment::Pro#3cessOutputs ()n ()" <<std::endl;
  NS_LOG_INFO ("Run Simulation.");VanetRoutingExperiment::ProcessOutputs ()
VanetRoutingExperiment::ProcessOutputs ()
  CheckThroughput ();VanetRoutingExperiment::ProcessOutputs ()
VanetRoutingExperiment::ProcessOutputs ()
  Simulator::Stop (Seconds (m_TotalSimTime));VanetRoutingExperiment::ProcessOutputs ()
  Simulator::Run ();VanetRoutingExperiment::ProcessOutputs ()
  Simulator::Destroy ();VanetRoutingExperiment::ProcessOutputs ()
}

// Prints actual position and velocity when a course change event occurs
void
VanetRoutingExperiment::
CourseChange (std::ostream *os, std::string context, Ptr<const MobilityModel> mobility) //what does the string context mean everywhere?
{
  ...
  double t = (Simulator::Now ()).GetSeconds ();#3
  if (t >= 1.0)#3
    {#3
      WaveBsmHelper::GetNodesMoving ()[nodeId] = 1;#3
    }#3
#3
  //NS_LOG_UNCOND ("Changing pos for node=" << nodeId << " at " << Simulator:#3:Now () );
...
{
  //IMPTT: 2 different functions GetWaveBsmStats and GetRoutingStats
  ...
  if (wavePktsSent > 0)
    {
      int wavePktsReceived = m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktCount ();
      wavePDR = (double) wavePktsReceived / (double) wavePktsSent; //check what exactly do the 2 values mean?? overall PDR??
    }
  ...
  //how does the distance affect ? Fading??
  double wavePDR1_2 = m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (1);


  //#3
  //in func VanetRoutingExperiment::ProcessOutputs () See #2, defined as shown below
//  double bsm_pdr1 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (1); //difference ??
...
below is the way overhead is calculated in VanetRoutingExperiment::ProcessOutputs (). SAME ????
...
*/
  ...
  out << (Simulator::Now ()).GetSeconds () << ","
      ...
      << mac_phy_oh << ""
      << std::endl;
//the above will be saved in file??
  ...
  for (int index = 1; index <= 10; index++)
    {
      m_waveBsmHelper.GetWaveBsmStats ()->SetExpectedRxPktCount (index, 0); //why is set to 0 after the simulation??
      m_waveBsmHelper.GetWaveBsmStats ()->SetRxPktInRangeCount (index, 0);
    }

  double currentTime = (Simulator::Now ()).GetSeconds ();
  if (currentTime <= (double) m_cumulativeBsmCaptureStart) //do I need to modify this stuff??
    {
      for (int index = 1; index <= 10; index++)
        {
  ...
  Simulator::Schedule (Seconds (1.0), &VanetRoutingExperiment::CheckThroughput, this); //use of reference??//
}
...
void
VanetRoutingExperiment::CommandSetup (int argc, char **argv) //IMPORTANT FUNCTION//
{
  CommandLine cmd;
  //PDR calculated at each of these distances
  double txDist1 = 50.0;
  ...
  cmd.AddValue ("80211Mode", "1=802.11p; 2=802.11b; 3=WAVE-PHY", m_80211mode); //#4
  cmd.AddValue ("traceFile", "Ns2 movement trace file", m_traceFile);
  cmd.AddValue ("logFile", "Log file", m_logFile);
  cmd.AddValue ("mobility", "1=trace;2=RWP", m_mobility); //#5
  ...
  cmd.AddValue ("gpsaccuracy", "GPS time accuracy, in ns", m_gpsAccuracyNs); //to be set or calculated??
...
VanetRoutingExperiment::ConfigureDefaults ()
{
  ...
  if (m_80211mode == 2) //80211Mode", "1=802.11p; 2=802.11b; 3=WAVE-PHY see #4
    {
...
{
  if (m_mobility == 1) //"mobility", "1=trace;2=RWP see #5
    {
      // Create Ns2MobilityHelper with the specified trace log file as parameter //so the trace file for the scenario has to be put where??
      Ns2MobilityHelper ns2 = Ns2MobilityHelper (m_traceFile);
      ns2.Install (); // configure movements for each node, while reading trace file
      // initially assume all nodes are not moving
      WaveBsmHelper::GetNodesMoving ().resize (m_nNodes, 0); //if new elements created to have a total of m_nNodes, 0 will be added to them
  ...
  else if (m_mobility == 2)
    {
      ...
      // we need antenna height uniform [1.0 .. 2.0] for loss model //next line
      pos.Set ("Z", StringValue ("ns3::UniformRandomVariable[Min=1.0|Max=2.0]"));

      Ptr<PositionAllocator> taPositionAlloc = pos.Create ()->GetObject<PositionAllocator> ();
      m_streamIndex += taPositionAlloc->AssignStreams (m_streamIndex); //what does the stream mean??
      /*AssignStreams  - 
      ...
      Parameters: stream - first stream index to use
      Returns: the number of stream indices assigned by this model
     */
      std::stringstream ssSpeed;
      ssSpeed << "ns3::UniformRandomVariable[Min=0.0|Max=" << m_nodeSpeed << "]"; //how is the function called from inside the string??//
      ...
      m_streamIndex += mobilityAdhoc.AssignStreams (m_adhocTxNodes, m_streamIndex); //doxygen says just 1 parameter passed??
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  if (m_lossModel == 3)
    {
      // two-ray requires antenna height (else defaults to Friss) //imptt
  ...
  //SetPcapDataLinkType - Set the data link type of PCAP traces to be used, but what are the options??
  //options - http://www.tcpdump.org/linktypes.html
  ...
  // SetRemoteStationManager - hold a list of per-remote-station state.
  wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager", //use constant rates for data and RTS transmissions 
                                                                //This class uses always the same transmission rate for every packet sent.
                                "DataMode",StringValue (m_phyModeB), //phyModeB means?
                                "ControlMode",StringValue (m_phyModeB));

  // Setup 802.11p stuff
  wifi80211p.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
                                      "DataMode",StringValue (m_phyMode),
                                      "ControlMode",StringValue (m_phyMode)); //phyMode means??
  ...
  // Set Tx Power
  wifiPhy.Set ("TxPowerStart",DoubleValue (m_txp)); //(name of the attribute, value of the attribute)
  wifiPhy.Set ("TxPowerEnd", DoubleValue (m_txp));
  wavePhy.Set ("TxPowerStart",DoubleValue (m_txp));
  wavePhy.Set ("TxPowerEnd", DoubleValue (m_txp));

  // Add an upper mac and disable rate control //why rate control disabled??
...
VanetRoutingExperiment::SetupWaveMessages ()
{
  ...
  if (m_80211mode == 3)
    {
      chAccessMode = 1; //made 0 for our case ??
    }

  m_waveBsmHelper.Install (m_adhocTxInterfaces,
                           Seconds (m_TotalSimTime),
                           m_wavePacketSize,
                           Seconds (m_waveInterval), //difference between m_TotalSimTime and m_waveInterval
                           // GPS accuracy (i.e, clock drift), in number of ns
                           SetRemoteStationManagerm_gpsAccuracyNs,
                           m_txSafetyRanges,
                           chAccessMode,
  ...
  m_streamIndex += m_waveBsmHelper.AssignStreams (m_adhocTxNodes, m_streamIndex); //what is the point of this index??
}
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 2)
    {
      ...
      m_traceFile = "src/wave/examples/low99-ct-unterstrass-1day.filt.7.adj.mob"; //engineering clinin YT channel has the method to create these files
      m_logFile = "low99-ct-unterstrass-1day.filt.7.adj.log";
      m_mobility = 1;
      m_nNodes = 99;
      m_TotalSimTime = 300.01;
