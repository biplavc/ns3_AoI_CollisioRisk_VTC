...
{
	return (2*3*sizeof(double)+2*sizeof(uint32_t)); // x and y coordinates both are doubles, IDs are uint32_t
}
...
void loc_header::Serialize (Buffer::Iterator start) const // from Adeel cam header
{
  ...
  start.WriteHtonU64 (ceil(m_velocity.x*100000)); // doubles are 8 bytes
	start.WriteHtonU64 (ceil(m_velocity.y*100000));
	start.WriteHtonU64 (ceil(m_velocity.z*100000));
...
uint32_t loc_header::Deserialize (Buffer::Iterator start)   // from Adeel cam header
{
  ...
  m_velocity.x = start.ReadNtohU64()/100000; // doubles are 8 bytes
	m_velocity.y = start.ReadNtohU64()/100000;
	m_velocity.z = start.ReadNtohU64()/100000;
...
{
  os << "Packet sent by Node "<<SenderId<<" from " << "(" << m_position.x << "," << m_position.y << "," << m_position.z << ") with velocity " <<"("<<m_velocity.x << "," << m_velocity.y << "," << m_velocity.z <<")"<<std::endl;
}
...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  Time sqhd;
  Time total_delay;
  while ((packet = socket->Recv ()) && (Simulator::Now()).GetInteger()/1000000000 > 174) // start logging after 173 seconds as in SUMO, all vehicles enter simulation after 173 seconds
  {
    ...
    double xx2 = mob2->GetPosition().x; double yy2 = mob2->GetPosition().y; //receiver values
    Ptr<ConstantVelocityMobilityModel> mob1 = Sender_Node->GetObject<ConstantVelocityMobilityModel>();
    // double xx1 = old_velocity.x; double yy1 = old_velocity.y; //sender old velocities
    // IMPTT: the above command was giving trouble so in the next line, I am taking the current velocities
    double xx1 = mob1->GetVelocity().x; double yy1 = mob1->GetVelocity().y; // double z_sender_vel = mob1->GetVelocity().z;
    ...
    double x_error = abs(xx-old_x); double y_error = abs(yy-old_y); 
    ...
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<sqhd.GetInteger()<<" "<<rcv.GetInteger()<<" "<<total_delay.GetInteger()<<" "<<xx<<" "<<old_x<<" "<<yy<<" "<<old_y<<" "<<total_error<<" "<<xx1<<" "<<yy1<<" "<<xx2<<" "<<yy2<<"\n"<<std::endl;
  }
}
                             ...
                             uint32_t pktCount, Time pktInterval )
{
  if (pktCount > 0)
    ...
    Vector vect2(x_old_vel, y_old_vel, z_old_vel);
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop for beacon rates
  {
    ...
    for (int m = 0; m < mm; m++) // loop for number of vehicles
    {
        ...
        //   double veh_spacing = ROAD_LENGTH_NUM*num_lanes/c.GetN();
          ...
          std::string file_name ("Prog_test_PF_10s_" + queue_here + "_" + str_2 + "Hz_" + str_1 + "U.txt"); 
          ...
          std::cout<<"PId TxId RxId GenTime RecTime Delay(ns) TxTruePos(X) TxHeadPos(X) TxTruePos(Y) TxHeadPos(Y) Error(m) Tx_HeadVelX Tx_HeadVelY RecvPos(X) RecvPos(Y)\n"<<std::endl;
          ...
          std::string loc_name ("Drop_test_PF_10s_" + queue_here + "_" + str_2 + "Hz_" + str_1 + "U.txt"); 
        ...
        //   {
          ...
          Ns2MobilityHelper ns2 = Ns2MobilityHelper ( "/home/biplav/mobility_circular_2.tcl");
