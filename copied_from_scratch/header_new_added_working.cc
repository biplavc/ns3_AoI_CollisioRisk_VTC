...
{
	return (3*sizeof(double)+1*sizeof(uint32_t)); // x and y coordinates both are doubles, ID is uint32_t
}
...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  {
    std::cout<<"Inside ReceivePacket "<<std::endl;
    SeqTsHeader seqTsx; 
    ...
    // Vector position = /NodeList/SenderId/$ns3::MobilityModel/$ns3::ConstantPositionMobilityModel.DoGetPosition();

    // //Ptr<MobilityModel> mob1 = c.Get(SenderId)->GetObject<MobilityModel>(); //problem1
    // // xx = mob1->GetPosition().x; yy = mob1->GetPosition().y; zz = mob1->GetPosition().z;
    // xx = position.x; yy = position.y; zz = position.z;
                             ...
                             uint32_t pktCount, Time pktInterval )
{
  if (pktCount > 0)
    {
    std::cout<<"Inside GenerateTraffic "<<std::endl;
    Ptr<Packet> p = Create<Packet> (pktSize-(12+24+4)); //12 bytes seqts, 28 bytes loc_header (24 for x,y,z and 4 for ID)
    loc_header new_header;
