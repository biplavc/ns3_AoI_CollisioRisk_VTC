...
uint32_t MsgCount; // will be used as MsgId to ID the packets from the tx side
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 800; //packets to be sent per node
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop over intervals
  {
    ...
    int arr[] = {150}; // denotes the number of vehicles
    ...
    for (int m = 0; m < mm; m++)
    {
        ...
        std::string file_name ("Final_q1_" + str_2 + "Hz_" + str_1 + "U.txt"); 
        ...
        std::string loc_name ("Drop" + str_2 + "Hz_" + str_1 + "U.txt"); 
        ...
        for(unsigned int i = 0; i < c.GetN(); i++)
        {
            ...
            curr_lane++;
            // std::cout<<Simulator::Now()<<" vehicle"<<i<<"'s position is ("<<posm.x<<", "<< posm.y <<", " <<posm.z<<"), speed = ("<<(mob->GetVelocity()).x<<", "<<(mob->GetVelocity()).y <<", " <<(mob->GetVelocity()).z<<")"<<std::endl;
