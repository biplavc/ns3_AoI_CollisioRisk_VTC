...
class loc_header : public Header 
{
    ...
    void SetSendTime (const uint64_t sendtime);
    const uint64_t GetSendTime() const;
...
{
	return (3*sizeof(double)+2*sizeof(uint32_t))+8; // x and y coordinates both are doubles, IDs are uint32_t, Sendtime is 8 bytes
}
...
void loc_header::SetSendTime (const uint64_t sendtime)
{
    SendTime = sendtime;
}
const uint64_t loc_header::GetSendTime() const
{
    return SendTime;
}
...
void ReceivePacket (Ptr<Socket> socket)
{
  ...
  while (packet = socket->Recv ())
  {
    ...
    uint64_t SendTime = new_header.GetSendTime();
    sender_q = 
    ...
    // Ptr<WifiMacQueue> sender_q = socket->GetNode()->GetDevice()->GetObject< WifiMacQueue>();
    std::cout<<
    ...
    std::cout<<MsgId<<" "<<SenderId<<" "<<socket->GetNode()->GetId()<<" "<<total_error<<" "<<sqhd.GetInteger()<<" "<<enqueue_time.GetInteger()<<" "<<dequeue_time.GetInteger()<<" "<<rcv.GetInteger()<<" "<<total_delay.GetInteger()<<" "<<xx<<" "<<old_x<<" "<<xx1<<" "<<xx2<<" "<<dequeue_time.GetInteger()-sqhd.GetInteger()<<" "<<rcv.GetInteger()-dequeue_time.GetInteger()<<" "<<SendTime<<"\n"<<std::endl;
    //         "PId          TxId                  RxId                      Error(m)            GenTime                 EnqueueTime                    DequeueTime                     RecTime                        Delay            TruePos(X) HeadPos(X)  Xvel    RecvPos(X)                      QueueingDelay                             Transmission Delay                    SendTime
                             ...
                             uint32_t pktCount, Time pktInterval )
{
  if (pktCount > 0)
    ...
    Ptr<Packet> p = Create<Packet> (pktSize-(12+24+8+8)); //12 bytes seqts, 24 bytes loc_header , 4+4 bytes IDs (MsgId and SenderId), 8 bytes SendTime
    loc_header new_header;
    ...
    int64_t current_time = (Simulator::Now()).GetInteger();
    new_header.SetSendTime(current_time);
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 10000; //packets to be sent per node
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop over intervals
  {
    ...
    int arr[] = {200}; // denotes the number of vehicles
    ...
    for (int m = 0; m < mm; m++)
    {
        ...
        pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=0.0|Max=100.0]")); // adjust as per ROAD_LENGTH_NUM
        ...
        std::string file_name ("Mod_q1_" + str_2 + "Hz_" + str_1 + "U.txt"); 
        ...
        std::cout<<"PId TxId RxId Error(m) GenTime EnqTime DeqTime RecTime Delay TruePos(X) HeadPos(X) Xvel RecvPos(X) QDelay TxDelay SendTime\n"<<std::endl;
        ...
        std::string loc_name ("Drop_q1_" + str_2 + "Hz_" + str_1 + "U.txt"); 
