...
// tag implementation from Avik's wave-80211p_risk_40n_GN_2.cc
...
NS_OBJECT_ENSURE_REGISTERED (MyTag);
 ...
 {
   return 4; // ID will be an integer so 4 bytes
 }
 ...
 uint32_t MyTag::GetSimpleValue (void) const
 {
   return node_ID;
 }
...
{
  os << "Position " << "( " << m_position.x << ", " << m_position.y << ", " << m_position.z << " )" << std::endl;
  ...
  while (packet = socket->Recv ())
    {
      ...
      // Ptr<MobilityModel> mob = socket->GetNode()->GetObject<MobilityModel>(); // will give the receiver's mobility
      // double x_actual = mob->GetPosition().x; double y_actual = mob->GetPosition().y; double z_actual = mob->GetPosition().z;

      // now get the sender's ID from the tag

	    //Ptr<MobilityModel> mobility = socket->GetNode()-> std::cout<<"actual positions are "<<x_actual<<", "<<y_actual<<", "<<z_actual<<std::endl;
      std::cout<<"Packet sent by node " <<senderID<< " and received by node "<< socket->GetNode()->GetId()<<" at "<<rcv<<", Packet generated at "<<sqhd<<" and overall delay for this packet is "<<total_delay<<std::endl;
  ...
  if (pktCount > 0)
    {
      ...
      // 2 headers added, now add the nodeID
      
      uint32_t node_ID = socket->GetNode()->GetId();
      MyTag tag;
      tag.SetSimpleValue(node_ID);
      p->AddPacketTag(tag);
