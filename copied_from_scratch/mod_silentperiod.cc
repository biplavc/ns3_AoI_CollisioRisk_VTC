                             ...
                             uint32_t pktCount, Time pktInterval )
{
  ...
  Time tx_start = Simulator::Now();
  uint64_t TX_START = tx_start.GetInteger();
  Time duration = Seconds(300); //fixed as per SAE
  uint64_t DURATION = duration.GetInteger();
  uint64_t TX_END = TX_START + DURATION;

  // std::cout<<"TX_END = "<<TX_END<<std::endl;
  if (TX_START < TX_END)
  {
    std::cout<<"TX_START = "<< TX_START/1000000000.000<<", TX_END = "<< TX_END/1000000000.000<<std::endl;
    ...
    // else
    //   Simulator::Schedule (( Simulator::Now().GetSeconds() + 3 ).GetSeconds(), &GenerateTraffic, socket, pktSize, pktCount - 1, pktInterval); //Once it enters the silent period zone, the next transmission is scheduled 3 seconds from then
  //   }
  // else
  //   {
  //   socket->Close ();
...
int main (int argc, char *argv[])
{
  ...
  uint32_t numPackets = 1000; // 300 for the other simulations
  ...
  // double intervals[] = {0.02857142857142857};
  // double intervals[] = {0.1 ,0.02};
  ...
  for (int yyy = 0; yyy <kk; yyy++) //loop over intervals
  {
    ...
    for (int m = 0; m < mm; m++)
    {
...

        {
          ...
          // std::cout<<"PId TxId RxId Error(m) GenTime RecTime Delay TruePos(X) HeadPos(X) Xvel RecvPos(X) \n"<<std::endl;
          ...
          // MAC tracing starts
          Simulator::Stop (Minutes (TotalTime));
